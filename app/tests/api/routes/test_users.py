...
def test_get_users_normal_user_me(
    ...
    r = client.get(f"{settings.API_PREFIX_STR}/users/me", headers=normal_user_token_headers)
    current_user = r.json()
    assert current_user
    assert current_user["is_active"] is True
...
def test_get_existing_user_permissions_error(
    ...
    r = client.get(
        f"{settings.API_PREFIX_STR}/users/999999",
...
def test_update_password_me(
    ...
    r = client.patch(
        f"{settings.API_PREFIX_STR}/users/me/password",
        headers=superuser_token_headers,
        json=data,
    )
    assert r.status_code == 200
...
def test_update_user(
    ...
    r = client.patch(
        f"{settings.API_PREFIX_STR}/users/{user.id}",
        headers=superuser_token_headers,
        json=data,
    )
    assert r.status_code == 200
...
def test_update_user_not_exists(
    ...
    data = {"full_name": "Updated_full_name"}
    r = client.patch(
        f"{settings.API_PREFIX_STR}/users/99999999",
        headers=superuser_token_headers,
        json=data,
    )
